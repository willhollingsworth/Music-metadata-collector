...
def run_tests():
    ...
    print(f'searching for string "{search_string}", found {len(results)} results')
    # pprint(format_search_results(results)) # show results
    # list artist ids from search
    print(
        "artist ids of results :",
        dict(Counter([x["result"]["primary_artist"]["id"] for x in results])),
    )
    # lists all songs from artist id
    results = get_all_songs(artist_id="582604")
    print(
        "get all songs: found {} songs by {}".format(
            len(results), results[0]["artist_names"]
        )
    )
